"""
Final pipeline for sepsis prediction using XGBoost. Includes patient-level utility metric,
...
3. Optimizes XGBoost hyperparameters using Optuna framework
...
def compute_patient_utility(
    ...
    """
    Compute patient-level utility by penalizing late/missed detections
    and rewarding early/optimal predictions. Returns total utility score.
    """
    if check_errors:
        ...
        if not all((p in (0, 1) for p in preds)):
            raise ValueError("preds must contain only 0/1 values.")
        if not (dt_early < dt_optimal < dt_late):
            raise ValueError("Require dt_early < dt_optimal < dt_late (relative to onset).")
...
def compute_normalized_utility(
    ...
    """
    Compute normalized utility across patients, comparing observed
    predictions with best-case and inaction scenarios.
    """
...
X = df.drop(columns=["Patient", "SepsisLabel"])
y = df["SepsisLabel"]
groups = df["Patient"]
...
def best_threshold(y_true, y_proba, alpha):
    """Identify optimal classification threshold based on a weighted trade-off between F1-score and recall.
    """
...
# Split into train/test ensuring patient-level stratification
...
for fold, (train_idx, val_idx) in enumerate(tqdm(cv.split(X_train, y_train, groups=groups_train), total=5, desc="Procesando pliegues")):
    ...
    try:
        ...
        util_fold = float('nan')
        warnings.warn(f"Utility error {fold}: {e}")
...
def objective(trial):
    ...
    # Apply sample weights to mitigate imbalance
...
# Determine best threshold after tuning
...
print(f"SHAP original values: {shap_values_all.shape} (per hour)")
print(f"SHAP values aggregated: {shap_values_patient.shape} (per patient)")

output_dir = "Results/XGBoost_Results"
os.makedirs(output_dir, exist_ok=True)

util_patients_df = pd.DataFrame({
    'Patient': util_details_test['patient_ids'],
...
util_patients_df.to_csv(os.path.join(output_dir, "utility_per_patient_test.csv"), index=False)
...
plt.tight_layout()
plt.savefig(os.path.join(output_dir, "shap_beeswarm.pdf"), dpi=300)
plt.close()
...
shap_importance_df.to_csv(os.path.join(output_dir, "shap_ranking_Patients.csv"), index=False)
...
plt.ylabel("")
plt.tight_layout()
plt.savefig(os.path.join(output_dir, "shap_top15_variables.pdf"), dpi=300)
plt.close()
